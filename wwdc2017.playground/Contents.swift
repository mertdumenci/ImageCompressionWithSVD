/*:
 ...
 I took Linear Algebra in my first semester in college (I'm a first year CS student),
 ...
 (My `Matrix` class uses Accelerate under the hood to compute the SVD and for matrix multiplications.)
*/
import UIKit
UI.setup()
...
/*:
 ...
 slow when executing complex programs. Because we're going to apply a multitude of complex
 arithmetic operations on the image, and we don't want this to be slow, we fetch and downsize
 the image first.
 ...
 a non-Playground environment).
 
 You can play around with the image and import your own if you want!
 (If you can't see the image, add a viewer for `image` in Playgrounds)
 */
...
/*:
 ...
 get the same image solely in the color that we picked. Thus, we can assemble 3 different matrices of colors
...
/*:
 ...
 is that every term in this equation has the `k`th singular value `σ_k` as a coefficient.
 ...
 Here we get the diagonal vector of the matrix `Σ` and plot them. (If you can't see the plot, add a viewer for `singularValue` in Playgrounds)
 */
...
/*:
 ...
 changes--Mathematics is a lot of fun! (If you can't see the image, add a viewer for `compressedImage` in Playgrounds)
 */
    compressedImage
/*:
 *: Linear Algebra actually deals with vector spaces and mappings between them.
 */
}
